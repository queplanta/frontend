...
import { Card, Chip, CardActionArea, CardContent, Divider,
  Grid, Paper, Typography, withStyles } from '@material-ui/core';
...
import TaxoClimb from '../plants/TaxoClimb.js';
...
(props) {
    ...
    revision_body = (<React.Fragment>
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={9}>
          ...
          {(plant.commonNames.edges.length > 0) && <div>
  ...
  if(object.__typename === 'Post') {
    ...
    revision_body = (<Grid container spacing={3}>
      <Grid item xs={12}>
  ...
  return <Width>
    ...
    <Card className={classes.cardRoot}>
      ...
      <Grid container spacing={1}>
        <Grid item xs={12} md={6}>{`${revision_pretty_text[revision.object.__typename]}: ${revision.id}`}
        </Grid>
        <Grid item xs={12} md={6} className={classes.textRight}>{current}</Grid>
        <Grid item xs={12} className={classes.revBody}>
          <Divider style={{marginBottom: 20}} />{revision_body}
        </Grid>
      </Grid>
      </CardContent>
    </Card>
