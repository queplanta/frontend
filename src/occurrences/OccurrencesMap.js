    ...
    <MapGeolocated {...otherProps}>
      <QueryRenderer
        ...
        render={({ error, props }) => {
          ...
          const {
            viewer: {
              allOccurrences: { edges: occurrences },
            },
          } = props;
          // if (error) {
            ...
            <React.Fragment>
              {occurrences.map(({ node: occurrence }) => {
                const mainImage = _.get(
                  occurrence.identity,
                  "images.edges[0].node.smallImage.url",
                  imgDefault
                );
                return (
                  <Marker
                    key={occurrence.id}
                    position={occurrence.location.coordinates}
                  >
                      ...
                      <div className={classes.card}>
                        ...
                        <div className={classes.details}>
                          <CardContent className={classes.content}>
                              ...
                              {occurrence.identity.commonName ? (
                                <React.Fragment>
                                  <Typography
                                    variant="body1"
                                    component="span"
                                    className={classes.commonName}
                                  >
                                    {occurrence.identity.commonName.name}
                                  </Typography>
                                  <br />
                            ...
                            <br />
                            <Link
                              href={`http://maps.google.com/?cbll=${occurrence.location.coordinates[0]},${occurrence.location.coordinates[1]}&cbp=12,90,0,0,5&layer=c`}
                              target="_blank"
